...
p1<-ggplot(filtered_data, aes(x=Gear, y=Country, fill = mean_errNV)) +
  ...
  facet_grid(rows = vars(filtered_data$Region),
             cols = vars(filtered_data$Length_Category), 
             scales = "free", 
             space="free_y") 
...
Plot Mean relative error for number of vessels as per p1 above in Shiny App. Remove faceting.  

```{r}

library(shiny)
runApp("/home/ubuntu/RousseauEtAl2023/Shinyapp")
